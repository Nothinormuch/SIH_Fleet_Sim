...
def test_release_profile_is_an_existing_selectable_stack():
    assert DEFAULT_ROUTE_POLICY == POLICY_BIOS_PIBT_V6
...
def test_grand_challenge_default_and_explicit_override():
    ...
    for scenario in SHOWCASE_SCENARIOS:
        expected = POLICY_BIOS_PIBT_V7 if scenario == "showcase_chokepoint" else DEFAULT_ROUTE_POLICY
...
def test_service_example_and_browser_launch_use_current_defaults():
    ...
    assert page.index('value="BIOS_PIBT.6"') < page.index('value="BIOS_PIBT.7"')
